...
def new_level(uid, msg):  # получение нового уровня( условия, кнопки выбора характеристик повышение параметров героя)
    user = users[uid]
    congrat = "🎉Ты получил новый уровень🎉\n\n" \
              "Твои характеристики:\n\n" \
              "Максимальное ХП: {0}❤\nСила: {1} 💪\nЗащита: {2} 🛡\n\n" \
              "Выбери какую хар-ку ты хочешь увеличить:".format(user.max_hp, user.power, user.defence)
    if msg == NEW_LVL:
        bot.send_message(uid, congrat, reply_markup=buttons_generator(NEW_LVL_BUTTONS))
        user.menu = NEW_LVL
